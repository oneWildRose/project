<template> <!-- 任务管理 -->
	<view class='hello'>
		...
		<div class="content">
			<ul v-if='index == 0'> <!-- 全部 -->
				<li v-for='(item, index) in list_cont' :key='index' @click='goAllot(item.status)'>
					<div class='tit'>
			...
			<ul v-if='index == 1'>
				<li v-for='(item, index) in list_cont' :key='index' v-if='item.status == 0' @click='goAllot(item.status)'>
					<div class='tit'>
						<text>任务类型：{{ item.type }}</text>
						<image :src="require('../../static/more(2).svg')" mode=""></image>
					</div>
			...
			<ul v-if='index == 2'>
				<li v-for='(item, index) in list_cont' :key='index' v-if='item.status == 1' @click='goAllot(item.status)'>
					<div class='tit'>
						<text>任务类型：{{ item.type }}</text>
						<image :src="require('../../static/more(2).svg')" mode=""></image>
					</div>
