		...
		<swiper style="height: 91%;margin: 10rpx auto;" :current="swiperCurrent" @transition="transition" @animationfinish="animationfinish" duration='300'>
			<swiper-item class="swiper-item">
				<scroll-view scroll-y lower-threshold='50' @scrolltolower='btmLeft'>
					<li v-for="(item, index) in listLeft" :key='index' @click='goPhotomsg(item.file_id, item.ispaizhao, item.id ,1)'>
			...
			<swiper-item class="swiper-item">
				<scroll-view scroll-y lower-threshold='50' @scrolltolower='btmRight'>
					<li v-for="(item, index) in listRight" :key='index' @click='goPhotomsg(item.file_id, item.ispaizhao, item.id,2)'>
						<image :src="item.plan_url" mode="" lazy-load></image>
						<div>
							<div>
								<text :class="item.name.length > 8? 'wid' : '' ">{{ item.name }}</text>
						...
						<view class='sta'>
							<p :class="item.state == 2? 'red' : 'blue' ">
								{{ item.state == 2? '审批拒绝' : '审批通过' }}
							</p>
							<!-- <p :class="item.iscl == 0? 'red' : 'blue' ">
								{{ item.iscl == 0? '未拍照' : '已拍照' }}
							</p> -->
	...
	export default {
		...
		methods: {
			...
			loadmoreLeft(page) {
				...
				page++
				this.$request('/api/lnspection/quality', {
					uid: this.uid,
					date: this.time,
			...
			goPhotomsg(file_id, ispaizhao, id, type) {
				if(ispaizhao == 0) {
					...
					uni.navigateTo({
						url: '../ind_provider/photoMsg/photoMsg?project_id=' + this.project_id + '&file_id=' + file_id + '&zhuid=' + id + '&time=' + this.time + '&direction=' + 'pz' + '&type=' + type
					})
				}
			}
		}
